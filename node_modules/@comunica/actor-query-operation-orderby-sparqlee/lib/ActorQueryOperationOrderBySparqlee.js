...
class ActorQueryOperationOrderBySparqlee extends bus_query_operation_1.ActorQueryOperationTypedMediated {
    ...
    async testOperation(operation, context) {
        ...
        for (let expr of operation.expressions) {
            ...
            const _ = new sparqlee_1.AsyncEvaluator(expr, bus_query_operation_1.ActorQueryOperation.getAsyncExpressionContext(context, this.mediatorQueryOperation));
        }
        return true;
    }
    async runOperation(operation, context) {
        ...
        for (let i = operation.expressions.length - 1; i >= 0; i--) {
            ...
            const sortedStream = new SortIterator_1.SortIterator(transformedStream, (left, right) => {
                let compare = (0, sparqlee_1.orderTypes)(left.result, right.result);
                if (!isAscending) {
                    compare *= -1;
                }
                return compare;
